...
const isArabic = (text) => { if (!text || typeof text !== 'string') return false; const arabicRegex = /[\u0600-\u06FF]/; return arabicRegex.test(text); };
const escapeXml = (text) => { if (typeof text !== 'string') return ''; return text.replace(/[<>&'"]/g, (char) => { switch (char) { case '<': return '&lt;'; case '>': return '&gt;'; case '&': return '&amp;'; case "'": return '&apos;'; case '"': return '&quot;'; default: return char; } }); };
...
app.post('/api/generate-word', async (req, res) => {
    try {
        ...
        let templateBuffer; try { const response = await fetch(WORD_TEMPLATE_URL); if (!response.ok) throw new Error(`Échec modèle Word (${response.status})`); templateBuffer = Buffer.from(await response.arrayBuffer()); } catch (e) { return res.status(500).json({ message: `Erreur récup modèle Word.` }); }
        const zip = new PizZip(templateBuffer);
        const doc = new Docxtemplater(zip, { paragraphLoop: true, nullGetter: () => "" });
        const groupedByDay = {}; const dayOrder = ["Dimanche", "Lundi", "Mardi", "Mercredi", "Jeudi"];
        const datesNode = specificWeekDateRangesNode[weekNumber];
        let weekStartDateNode = null; if (datesNode?.start) weekStartDateNode = new Date(datesNode.start + 'T00:00:00Z');
        ...
        const joursData = dayOrder.map(dayName => {
            ...
            if (groupedByDay[dayName] && groupedByDay[dayName].length > 0) {
                ...
                const matieres = sortedEntries.map(item => {
                    const fieldsToProcess = { Lecon: item[leconKey], travailDeClasse: item[travauxKey], Support: item[supportKey], devoirs: item[devoirsKey] };
                    const processedFields = { matiere: item[matiereKey] ?? "" };
                    for (const key in fieldsToProcess) {
                        ...
                        if (rawText && typeof rawText === 'string' && rawText.trim() !== '') {
                            ...
                            lines.forEach((line, index) => {
                                ...
                                if (index < lines.length - 1) { contentXml += `<w:r><w:br/></w:r>`; }
                            });
        ...
        if (datesNode?.start && datesNode?.end) { const startD = new Date(datesNode.start + 'T00:00:00Z'), endD = new Date(datesNode.end + 'T00:00:00Z'); if (!isNaN(startD.getTime()) && !isNaN(endD.getTime())) { plageSemaineText = `du ${formatDateFrenchNode(startD)} au ${formatDateFrenchNode(endD)}`; } }
        const templateData = { classe: classe, semaine: weekNumber, plageSemaine: plageSemaineText, notes: notes || "", jours: joursData };
        doc.render(templateData);
        const buf = doc.getZip().generate({ type: 'nodebuffer', compression: 'DEFLATE' });
        const filename = `Plan_hebdomadaire_S${weekNumber}_${classe.replace(/[^a-z0-9]/gi, '_')}.docx`;
        res.setHeader('Content-Disposition', `attachment; filename="${filename}"`); res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'); res.send(buf);
...
app.post('/api/generate-ai-lesson-plan', async (req, res) => {
    try {
        ...
        if (englishTeachers.includes(enseignant)) {
            prompt = `As an expert pedagogical assistant, create a detailed 45-minute lesson plan in English...`; // Prompt inchangé
        } else if (arabicTeachers.includes(enseignant)) {
            prompt = `بصفتك مساعدًا تربويًا خبيرًا، قم بإنشاء خطة درس مفصلة باللغة العربية مدتها 45 دقيقة...`; // Prompt inchangé
        } else {
            prompt = `En tant qu'assistant pédagogique expert, crée un plan de leçon détaillé de 45 minutes en français...`; // Prompt inchangé
        }
        ...
        const templateData = {
            ...aiData,
            Semaine: week, Lecon: lecon, Matiere: matiere, Classe: classe, Jour: jour, Seance: seance, NomEnseignant: enseignant,
            Date: formattedDate, // On utilise la date calculée
        ...
        res.setHeader('Content-Disposition', `attachment; filename="${filename}"`); res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'); res.send(buf);
    } catch (error) { console.error('❌ Erreur serveur /generate-ai-lesson-plan:', error); if (!res.headersSent) { const errorMessage = error.message || "Erreur interne."; res.status(500).json({ message: `Erreur interne lors de la génération IA: ${errorMessage}` }); } }
});
