...
app.post('/api/generate-word', async (req, res) => {
    try {
        ...
        // Ancien code créait une structure imbriquée que le template ne comprenait pas.
        // Le nouveau code "aplatit" les données en concaténant les informations de chaque jour
        // dans une seule chaîne de caractères, ce qui correspond au template.
        const joursData = dayOrder.map(dayName => {
            ...
            // Concaténer les données de toutes les entrées du jour dans des chaînes de caractères uniques avec sauts de ligne
            const matiere = sortedEntries.map(item => item[matiereKey] ?? "").join('\n');
            const Lecon = sortedEntries.map(item => item[leconKey] ?? "").join('\n');
            const travailDeClasse = sortedEntries.map(item => item[travauxKey] ?? "").join('\n');
            const Support = sortedEntries.map(item => item[supportKey] ?? "").join('\n');
            const devoirs = sortedEntries.map(item => item[devoirsKey] ?? "").join('\n');
